...
window.onload = function () {
		...
		var eventCallback = {
			ui: function(data) {
				...
				else if (config.showServerLogo == false) { document.querySelector('.info.player').style.bottom = '20px'; }
			...
			createStatus: function(data) {
				...
				var statusIcon = data.icon + '<span style="background: linear-gradient(90deg, '+statusSecondaryColor+' 0%, '+statusPrimaryColor+' 100%);"></span>';
			...
			setStatus: function(data) {
				...
				if (data.isdead == true) {
					...
					for (i = 0; i < data.status.length; i++) {
						...
						if (document.querySelector('#'+data.status[i].name+' span')) { document.querySelector('#'+data.status[i].name+' span').style.width = statusValue+'%'; }
						if (statusValue <= 35) {
