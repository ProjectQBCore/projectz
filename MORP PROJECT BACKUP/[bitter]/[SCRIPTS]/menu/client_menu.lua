...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsDisabledControlPressed(0, keybindControls[keyBind]) and GetLastInputMethod(2) then--and not exports['police']:IsHandcuffed() then
            showMenu = true
            local enabledMenus = {}
...
RegisterNetEvent('needbackup')
AddEventHandler('needbackup', function()
  local streetLabel = GetStreetNameFromHashKey(GetStreetNameAtCoord(GetEntityCoords(PlayerPedId()).x, GetEntityCoords(PlayerPedId()).y, GetEntityCoords(PlayerPedId()).z))
  local coords = GetEntityCoords(PlayerPedId())
  TriggerServerEvent('policealerts:server:AddPoliceAlert', {
    timeOut = 10000,
    alertTitle = "Officer needs backup.",
    coords = {
        x = coords.x,
        y = coords.y,
        z = coords.z,
...
RegisterNetEvent('clamp')
AddEventHandler('clamp', function()
    ExecuteCommand('clamp')
end)
            ...
            ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
                if quantity > 0 then
                    ...
                    if IsPedDeadOrDying(closestPlayerPed, 1) then
                        ...
                        Progressbar("revive_player", "Reviving Player..", 5000, false, false, {
...
--[[
                        ...
                        TriggerEvent('notification', 'You revived '..GetPlayerName(closestPlayer)..' !', 1)
                    else
                        TriggerEvent('notification', 'That player is not conscious!', 2)
                    end
                else
                    TriggerEvent('notification', 'You dont have medikit', 2)
                end
                IsEMSBusy = false
            end, 'medikit')
        ...
        elseif currentAction == "billcustom" then
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
                title = 'Bill Amount'
            },function(data, menu)
                local amount = tonumber(data.value)
...
AddEventHandler('weapon:equip', function()
    ExecuteCommand('weapon')
end)

RegisterNetEvent('cred:menuop')
AddEventHandler('cred:menuop', function()
    ExecuteCommand('credits')
end)

RegisterNetEvent('xp:checkop')
AddEventHandler('xp:checkop', function()
    ExecuteCommand('ESXP')
end)

RegisterNetEvent('mech:inspectop')
AddEventHandler('mech:inspectop', function()
    ExecuteCommand('inspect')
end)

RegisterNetEvent('crew:menuop')
AddEventHandler('crew:menuop', function()
    ExecuteCommand('crew')
end)

RegisterNetEvent('crew:joincrewop')
AddEventHandler('crew:joincrewop', function()
    ExecuteCommand('joincrew')
end)

RegisterNetEvent('crew:createcrewop')
AddEventHandler('crew:createcrewop', function()
    ExecuteCommand('createcrew')
end)

RegisterNetEvent('crew:leftcrewop')
AddEventHandler('crew:leftcrewop', function()
    ExecuteCommand('leftcrew')
end)

RegisterNetEvent('crew:deletecrewop')
AddEventHandler('crew:deletecrewop', function()
    ExecuteCommand('deletecrew')
end)

RegisterNetEvent('crew:rankop')
AddEventHandler('crew:rankop', function()
    ExecuteCommand('rank')
end)
...
RegisterNetEvent('clean_vehicle')
AddEventHandler('clean_vehicle', function()
    ...
    if not IsPedInAnyVehicle(ped, false) then vehicle = GetClosestVehicle(coords.x, coords.y, coords.z, 5.0, 0, 71) end
        ...
        if Vdist2(GetEntityCoords(PlayerPedId()), GetEntityCoords(vehicle)) < 5 then
            ...
            TriggerEvent('notification', 'Where is the vehicle??')
        end
    else
