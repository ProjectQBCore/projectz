...
function getCraftingLevel(identifier)
    return tonumber(
        MySQL.Sync.fetchScalar(
            ...
            "SELECT `rp_rank` FROM users WHERE identifier = @identifier ",
...
ESX.RegisterUsableItem('ct', function(source)
    local xPlayer = ESX.GetPlayerFromId(source)
    xPlayer.removeInventoryItem('ct', 1)
    TriggerClientEvent('smoke:crafting', source)    
end)

RegisterServerEvent('smoke:create')
AddEventHandler('smoke:create',function()
    TriggerClientEvent("smoke-craftspawn",source, Config.ObjectsLoot)
end)