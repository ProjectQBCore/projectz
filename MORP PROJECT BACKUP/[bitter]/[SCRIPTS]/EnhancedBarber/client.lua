...
Citizen.CreateThread(function()
	while true do
		...
		for k, loc in pairs(Config.Shops) do
			...
			if distance < 30 then
				...
				if not isPedLoaded then
					...
					RequestModel(GetHashKey("BF4RusEngineer"))
					while not HasModelLoaded(GetHashKey("BF4RusEngineer")) do
						Wait(1)
					end
					npc = CreatePed(4, GetHashKey("BF4RusEngineer"), loc.x, loc.y, loc.z - 1.0, loc.rot, false, true)
		...
		if (isPedLoaded and not isNearShop) then
            DeleteEntity(npc)
			SetModelAsNoLongerNeeded(GetHashKey("BF4RusEngineer"))
			isPedLoaded = false
		end
