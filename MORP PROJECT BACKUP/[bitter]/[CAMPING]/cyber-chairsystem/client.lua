        ...
        if isWithinObject and oElement.fObject ~= 0 then
            ...
            if (distanceDiff < 2.0 and not InUse) then
                if (oElement.fObjectIsBed == true) then
                    ...
                    if IsControlJustPressed(0, Config.objects.ButtonToLayOnBed) then
                        TriggerServerEvent('cyber-chairsystem:Server:Enter', oElement, oElement.fObjectCoords)
                    end
                else    
            ...
            if (InUse) then
                ...
                if IsControlJustPressed(0, Config.objects.ButtonToStandUp) then
                    InUse = false
                    TriggerServerEvent('cyber-chairsystem:Server:Leave', oElement.fObjectCoords)
...
RegisterNetEvent('cyber-chairsystem:Client:Animation')
AddEventHandler('cyber-chairsystem:Client:Animation', function(v, coords)
