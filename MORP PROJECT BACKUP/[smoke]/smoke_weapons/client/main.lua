...
function OpenMainMenu(zone)
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'wea_cat',
    {
      ...
      elements = {
        { label = "Weapons Class: D", value = 'wl_1' },
        { label = "Weapons Class: C", value = 'wl_2' },
        { label = "Weapons Class: B", value = 'wl_3' },
        { label = "Weapons Class: A", value = 'wl_4' },
        { label = "Weapons Class: S", value = 'wl_5' },
      }
    },
    function (data, menu)
      ...
      if data.current.value == 'wl_1' then
        if exports.esx_xp:ESXP_GetRank() >= 1 then -- Licenses['weapon'] or Licenses['weapon2'] or Licenses['weapon3'] or Licenses['weapon4'] or Licenses['weapon5'] then
          OpenShopMenu(rvalue, zone)
        else
          OpenBuyLicenseMenu(rvalue, zone)
        end
      elseif data.current.value == 'wl_2' then
        if exports.esx_xp:ESXP_GetRank() >= 26 then -- Licenses['weapon2'] or Licenses['weapon3'] or Licenses['weapon4'] or Licenses['weapon5'] then
          OpenShopMenu(rvalue, zone)
        else
          OpenBuyLicenseMenu(rvalue, zone)
        end
      elseif data.current.value == 'wl_3' then
        if exports.esx_xp:ESXP_GetRank() >= 51 then -- Licenses['weapon3'] or Licenses['weapon4'] or Licenses['weapon5'] then
          OpenShopMenu(rvalue, zone)
        else
          OpenBuyLicenseMenu(rvalue, zone)
        end
      elseif data.current.value == 'wl_4' then
        if exports.esx_xp:ESXP_GetRank() >= 76 then -- Licenses['weapon4'] or Licenses['weapon5'] then -- A Class
          OpenShopMenu(rvalue, zone)
        else
          OpenBuyLicenseMenu(rvalue, zone)
        end
      elseif data.current.value == 'wl_5' then -- S Class
        if exports.esx_xp:ESXP_GetRank() >= 101 then -- Licenses['weapon5'] then
          OpenShopMenu(rvalue, zone)
        else
          OpenBuyLicenseMenu(rvalue, zone)
        end
...
function OpenBuyLicenseMenu(rvalue, zone)
  ...
  elseif rvalue == 'wl_3' then
    ...
    wtype = 'weapon3'
  elseif rvalue == 'wl_4' then
    price = LicensePrice * 4
    wtype = 'weapon4'
  elseif rvalue == 'wl_5' then
    price = LicensePrice * 5
    wtype = 'weapon5'
  end
